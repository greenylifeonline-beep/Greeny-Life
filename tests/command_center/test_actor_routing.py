...
def test_all_requires_presence_binding_and_live_consumer(tmp_path):
    ...
    presence.write_text(json.dumps({"seats": {
        ...
        "C12": {"presence": "PRESENT", "lease_expires_at": iso(-5)}}}), encoding="utf-8")
    ...
    assert "C12" not in out["targets"] and "C2" not in out["targets"]
...
def test_bound_without_consumer_is_not_auto_routable(tmp_path):
    ...
    presence = tmp_path / "presence.json"
    presence.write_text(json.dumps({"seats": {"C6": {"presence": "PRESENT", "lease_expires_at": iso(5)}}}), encoding="utf-8")
    bindings = tmp_path / "bindings.json"
