...
class WorkStealingScheduler:
    ...
    def load_resource_projection(
        self,
        path: str | None = None,
    ) -> dict[str, Any]:
        """
        Read resource evidence only.
        The scheduler MUST NOT probe cloud providers.
        """
        import json
    ...
    def resource_selection_factors(
        self,
    ) -> list[str]:
        """
        Reuse A14 selection semantics without activating
        A14 as a second scheduler.
        """
        import json
        from pathlib import Path
        ...
        if router.exists():
            try:
                ...
                # Routing must remain usable even if the
                # optional semantic source is unreadable.
                pass
    ...
    def route_resource_task(
        ...
        *,
        ...
        """
        Capability/risk eligibility first.
        Economics only rank workers that are already eligible.
        """
        ...
        data=(
            ...
            else self.load_resource_projection(
                projection_path
            )
        )
        ...
        if (
            risk_class
            in {"HIGH","CRITICAL"}
            and minimum_accuracy is None
        ):
            return {
                "ok":False,
                "reason":
                    "RISK_ACCURACY_REQUIREMENT_UNPROVEN",
                "eligible":[],
                "rejected":[],
                "selection_factors":
                    self.resource_selection_factors(),
            }
        ...
        for row in records:
            ...
            if (
                ...
                if actual is None:
                    ...
                    reason=(
                        "VERIFIED_ACCURACY_BELOW_THRESHOLD"
                    )
        ...
        def score(
            ...
            if (
                ...
                if (
                    ...
                    and requested_locality
                    in locality
                ):
                    locality_match=1


            # Quality and evidence remain dominant.
