...
$App=Join-Path $RuntimeRoot "app";$Logs=Join-Path $RuntimeRoot "logs";$Pkg=Join-Path $App "raios\command_center";$SearchPkg=Join-Path $App "raios\search_cortex";$ResourcePkg=Join-Path $App "raios\resource_fabric";$GatewayPkg=Join-Path $App "raios\ai_gateway";$CouncilOpsPkg=Join-Path $App "raios\council_ops";$A2APkg=Join-Path $App "raios\a2a";$Mcp=Join-Path $App "raios_mcp"
...
New-Item -ItemType Directory -Force -Path $Pkg,$SearchPkg,$ResourcePkg,$GatewayPkg,$CouncilOpsPkg,$A2APkg,$Mcp,$Logs,(Join-Path $App "raios")|Out-Null
...
Copy-Item(Join-Path $Repo "src\raios\council_ops\*")$CouncilOpsPkg -Recurse -Force
Copy-Item(Join-Path $Repo "src\raios\a2a\*")$A2APkg -Recurse -Force
