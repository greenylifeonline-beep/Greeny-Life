...
def train(*, auto: bool, full: bool) -> dict:
    ...
    # P3: reuse existing self-inspection engine inside existing train cycle.
    from RAIOS.V9.autonomic.self_inspection.engine import inspect as self_inspect

    inspection_context = {
        "source": "C5_TRAIN_MESH",
        "host": current,
        "raw_host": raw_host,
        "auto": auto,
        "full": full,
        "keepers_run": list(ran),
        "p0_knowledge_e2e": True,
    }

    self_inspection = self_inspect(
        ROOT,
    ...
    ran.append({
        ...
        "ok": self_inspection.get("self_inspection_proven") is True,
    })
