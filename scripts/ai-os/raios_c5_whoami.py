...
def control_plane_runtime() -> dict:
    ...
    return {
        ...
        "public_url": f"http://{bind_host}:8765",
        "c1_url": f"http://{bind_host}:8876",
        "go_windows": "powershell -ExecutionPolicy Bypass -File scripts/ai-os/raios_c5_screen.ps1 -Go",
...
def write_screen_home_receipt(bind: dict | None = None) -> dict:
    ...
    rec = {
        ...
        "c1": home["c1"],
        "lanes": {"PUBLIC": 8765, "C1": 8876},
        "go_windows": home.get("go_windows"),
        "mcp": home["mcp"],
        "install_windows": home["install_windows"],
        "ensure_windows": home["ensure_windows"],
        "ensure_linux": home["ensure_linux"],
...
def render_md(rec: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            f"- شاشة الجميع (PUBLIC): `{bind.get('c5_screen_public') or bind.get('c5_screen_default') or 'http://127.0.0.1:8765'}`",
            f"- شاشة C1 فقط: `{bind.get('c5_screen_c1') or bind.get('c5_base_c1') or 'http://127.0.0.1:8876'}` — نفس C5، ليس C5 ثاني",
