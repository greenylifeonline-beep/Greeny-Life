...
def main() -> int:
    ...
    check("C0_SEAT_ABOLISHED" in LAWS, "C0 seat abolished")
    check("C1_INSTANCE_IS_CURSOR" in LAWS, "C1 instance is Cursor")
    check("C3_SEAT_IS_CHATGPT_PEER" in LAWS, "C3 is ChatGPT peer")
    check("C5_SEAT_IS_RAIOS" in LAWS, "C5 is RAIOS")
    check("C5_IS_C1_LOYAL_ASSISTANT" in LAWS, "C5 is C1 loyal assistant")
    check("ABSORB_DIGEST_NE_WAL_DUMP" in LAWS, "absorb is digest not WAL dump")
    check("REPAIR_EXECUTOR_NE_C_SEAT" in LAWS, "Repair is unseated")
    check("LOCAL_MCP_RENDEZVOUS_NE_REMOTE_MEETING" in LAWS, "local meeting is not remote")
    check("C5_GRANT_IS_PERMANENT" in LAWS, "C5 grant is permanent")
    check("LEARN_THEORY_THEN_PRACTICE_85" in LAWS, "theory then 85 percent practice")
    check("PATHOLOGY_COMPELS_REPAIR" in LAWS, "pathology compels repair")
    check("FATHER_SON_BIND_SAME_LAWS" in LAWS, "father and son bind same laws")
