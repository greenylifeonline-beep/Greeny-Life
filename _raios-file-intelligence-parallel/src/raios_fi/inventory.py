...
def reuse_map(repo: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "wrapped": [
            ...
            {"name": "magika", "role": "preferred type detector; WRAP present, binary missing", "action": "WRAP"},
            {"name": "tika", "role": "document extract; WRAP present, jar missing, no OCR", "action": "WRAP"},
            {"name": "jq", "role": "JSON config canonical diff when present"},
            {"name": "yq", "role": "YAML config canonical diff when present"},
        ],
        "created_in_this_package": [
            "File Intelligence SPI",
            "FileObject + deterministic ids",
            "Magika/Tika detect-only adapters",
...
def gap_map(tools: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "gaps": [
            {
                "id": "magika",
                "impact": "content-type accuracy",
                "fallback": "signature+parser-probe",
                "status": "ADAPTER_PRESENT_BINARY_MISSING",
                "action": "WRAP",
                "install": False,
            },
            {
                "id": "tika",
                "impact": "PDF/Office extraction",
                "fallback": "UNAVAILABLE status, no OCR",
                "status": "ADAPTER_PRESENT_BINARY_MISSING",
                "action": "WRAP",
                "install": False,
                "note": "java present is not tika",
            },
            {"id": "tree-sitter", "impact": "TS/JS/PS structural parse", "fallback": "regex heuristic", "status": "MISSING"},
            {
                "id": "universal-ctags",
                "impact": "multi-language symbols",
                "fallback": "python ast only",
                "status": "MISSING",
                "note": "GNU Emacs ctags rejected",
            },
            {
                "id": "ast-grep",
                "impact": "structural search",
                "fallback": "ripgrep+symbols",
                "status": "MISSING",
                "note": "/usr/bin/sg is Linux set-group, not ast-grep",
            },
